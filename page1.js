window.onload = function() {
    adjustStickerGridPosition(); // 确保在所有资源加载完成后调用此函数
};
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function filterStickers() {
        ...
        document.querySelectorAll('.sub-filter div.selected').forEach(sel => {
            ...
            if (category === 'botanical') { // 确保使用正确的属性名
...
const rightSide = document.querySelector('.right-side');
    ...
    rightSide.addEventListener('drop', function(event) {
        ...
        const offsetX = event.clientX - rect.left; // X坐标
        const offsetY = event.clientY - rect.top; // Y坐标
